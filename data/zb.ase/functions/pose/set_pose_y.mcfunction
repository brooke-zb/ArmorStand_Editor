...
execute store result storage zb:ase Pose[1] float 1 run scoreboard players get @s zb.ase_pose_y

# special case #
execute if score @s zb.ase_opt_part matches 1..2 if score @s zb.ase_pose_y matches 0 run data modify storage zb:ase Pose[1] set value 0.001f
...
# reset score #
scoreboard players reset @s zb.ase_pose_y
